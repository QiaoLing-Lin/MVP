...
from captioning.data.dataloader_ff import *
import skimage.io
import eval_utils
import captioning.utils.misc as utils
from captioning.utils.rewards import init_scorer, get_self_critical_reward
from captioning.modules.loss_wrapper import LossWrapper_  # LossWrapper (feat fusion)
...
def train(opt):
    ...
    lw_model = LossWrapper_(model, opt)
    ...
    ##########################
    if opt.noamopt:
        assert opt.caption_model in ['transformer', 'bert', 'm2transformer', 'clipffprefixbert'], 'noamopt can only work with transformer'   # add feat fusion model
    ...
    try:
        while True:
            ...
            tmp = [data['fc_feats'], data['att_feats'], data['rsi_feats'], data['labels'], data['masks'], data['att_masks'], data['rsi_masks']]   # rsi_feats
            ...
            model_out = dp_lw_model(fc_feats, att_feats, rsi_feats, labels, masks, att_masks, rsi_masks, data['gts'], torch.arange(0, len(data['gts'])), sc_flag)
            ...
            if (iteration % opt.save_checkpoint_every == 0 and not opt.save_every_epoch) or \
                ...
                val_loss, predictions, lang_stats = eval_utils.eval_split(
                        dp_model, lw_model.crit, loader, opt, eval_kwargs)
