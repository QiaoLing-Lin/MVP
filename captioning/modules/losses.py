...
class LanguageModelCriterion(nn.Module):
    ...
    def forward(self, input, target, mask, reduction='mean'):
        if target.ndim == 3:
            target = target.reshape(-1, target.shape[2])  # [bs*5, 21]
            mask = mask.reshape(-1, mask.shape[2])  # [bs*5, 21]
        N,L = input.shape[:2]  # [bs*5, 21]
