    ...
    for i in range(N):
        ...
        for j in range(D):
            ...
            if ix > 0 :
                if j >= 1:
                    txt = txt + ' '
                if (ix_to_word[str(ix.item())] == 'MASK') or (ix_to_word[str(ix.item())] == '[MASK]'):
                    txt = txt.rstrip()
                else:
...
def build_optimizer(params, opt):
    if opt.optim == 'rmsprop':
        ...
        return RAdam(params, opt.learning_rate, (opt.optim_alpha, opt.optim_beta), opt.optim_epsilon, weight_decay=opt.weight_decay)
    else:
