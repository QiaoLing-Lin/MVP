...
class HybridLoader:
    ...
    def __init__(self, db_path, ext, in_memory=False):
        ...
        if self.ext == '.npy':
            ...
            def load_npz(x):
                ...
                if 'features' in x:  # clip feature [features, g_feature]
                    return x['features']   # normally it should be 'feat', but under cocotest_bu, the key is saved to be 'z' mistakenly.
                elif 'feat' in x:
                    return x['feat']
    ...
    def get(self, key):
        ...
        if self.in_memory and key in self.features:
            ...
            f_input = open(os.path.join(self.db_path, key + self.ext[:-3]), 'rb').read()  # g_feature
        else:
...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.opt.input_label_h5 != 'none':
            ...
            # if 'textmask' in self.opt.id:
            if self.opt.use_text_mask:
                self.mask_label = self.h5_label_file['mask_labels'][:]  # mask
        ...
        if 'clip' in opt.input_att_dir:  # clip["g_features"]
            if opt.caption_model in ['updown', 'fc']:
                self.fc_loader = HybridLoader(self.opt.input_att_dir, '.npz_fc', in_memory=self.data_in_memory)
        else:
        ...
        # if self.opt.use_feat_fusion:
        self.rsi_loader = HybridLoader(self.opt.input_att_dir_rsi, '.npz', in_memory=self.data_in_memory)  # feature fusion
    ...
    def get_captions(self, ix, seq_per_img):
        ...
        if ncap < seq_per_img:
            ...
            for q in range(seq_per_img):
                ...
                # if 'textmask' in self.opt.id:
                if self.opt.use_text_mask:
                    seq[q, :] = self.mask_label[ixl, :self.seq_length]  # mask
                else:
                    seq[q, :] = self.label[ixl, :self.seq_length]
        else:
            ...
            # if 'textmask' in self.opt.id:
            if self.opt.use_text_mask:
                seq = self.mask_label[ixl: ixl + seq_per_img, :self.seq_length]  # mask
            else:
                seq = self.label[ixl: ixl + seq_per_img, :self.seq_length]
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            # fetch image
            tmp_fc, tmp_att, tmp_rsi, tmp_seq, \
                ix, it_pos_now, tmp_wrapped = sample   # tmp_rsi feature fusion
            if tmp_wrapped:
                wrapped = True

            fc_batch.append(tmp_fc)
            att_batch.append(tmp_att)
            rsi_batch.append(tmp_rsi)  # feature fusion
        ...
        fc_batch, att_batch, rsi_batch, label_batch, gts, infos = \
            zip(*sorted(zip(fc_batch, att_batch, rsi_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True))  # feature fusion
        data = {}
        data['fc_feats'] = np.stack(fc_batch)
        # merge att_feats
        max_att_len = max([_.shape[0] for _ in att_batch])
        max_rsi_len = max([_.shape[0] for _ in rsi_batch])  # feature fusion

        data['att_feats'] = np.zeros([len(att_batch), max_att_len, att_batch[0].shape[1]], dtype = 'float32')
        for i in range(len(att_batch)):
            data['att_feats'][i, :att_batch[i].shape[0]] = att_batch[i]

        data['rsi_feats'] = np.zeros([len(rsi_batch), max_rsi_len, rsi_batch[0].shape[1]], dtype="float32")  # feature fusion
        for i in range(len(rsi_batch)):
            data['rsi_feats'][i, :rsi_batch[i].shape[0]] = rsi_batch[i]

        data['att_masks'] = np.zeros(data['att_feats'].shape[:2], dtype='float32')
        for i in range(len(att_batch)):
            data['att_masks'][i, :att_batch[i].shape[0]] = 1

        data['rsi_masks'] = np.zeros(data['rsi_feats'].shape[:2], dtype='float32')   # feature fusion
        for i in range(len(rsi_batch)):
            data['rsi_masks'][i, :rsi_batch[i].shape[0]] = 1
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            ...
            rsi_feat = self.rsi_loader.get(str(self.info['images'][ix]['id']))  # feature fusion

            # Reshape to K x C
            att_feat = att_feat.reshape(-1, att_feat.shape[-1])
            rsi_feat = rsi_feat.reshape(-1, rsi_feat.shape[-1])   # feature fusion
        ...
        if self.use_fc:
            try:
                ...
                # print("fc_feat's shape:", fc_feat.shape)
                # raise Exception("att")
        else:
        ...
        return (fc_feat,
                att_feat, rsi_feat, seq,    # feature fusion
