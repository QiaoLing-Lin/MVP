...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, prefix_feat, init_state, init_logprobs, *args, **kwargs):  # add prefix feat
        # set_trace()
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size): 
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    if prefix_feat is not None:
                        logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it, prefix_feat=prefix_feat, *(args[divm] + [state_table[divm]]))   # add prefix feat
                    else:
                        logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it, *(args[divm] + [state_table[divm]]))  # add prefix feat
