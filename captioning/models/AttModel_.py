...
def get_split_features(x, split_dim, n_splits):
    split_dim = split_dim
    split_features = torch.zeros(x.size(0), n_splits, split_dim).cuda()
    for i in range(x.shape[0]):

        for j in range(n_splits):
            start_idx = split_dim * j
            end_idx = split_dim * j + split_dim
            split_features[i][j] = x[i, start_idx:end_idx]
...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        # self.embed = nn.Sequential(nn.Embedding(self.vocab_size, self.input_encoding_size),   # GPT2Tokenizer
                                   nn.ReLU(),
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, prefix_feat=None, output_logsoftmax=1):
        # 'it' contains a word index
        # set_trace()
        xt = self.embed(it)
        # set_trace()

        if self.opt.use_prefix_feat is True:
            output, state = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, prefix_feat)  # prefix feat
        else:
            output, state = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks)   # yuan

        if output_logsoftmax:
            logprobs = F.log_softmax(self.logit(output), dim=1)   # GPT2
        else:
    ...
    def _sample_beam(self, fc_feats, att_feats, rsi_feats, att_masks=None, rsi_masks=None, opt={}):
        ...
        # att_feats_, rsi_feats_, s, am, rm, sm = self._prepare_feature_forward(att_feats, rsi_feats, att_masks, rsi_masks)  # linear add prefix feat (att_feats_, rsi_feats_)
        if opt.get('use_prefix_feat'):  # add prefix feat
            prefix_clip = self.adapool(att_feats)  # linear  att_feats_
            prefix_rn = self.adapool(rsi_feats)   # linear  rsi_feats_
        else:
            prefix_clip = prefix_rn = None

        # att_feats_, rsi_feats_, s, am, rm, sm = self._prepare_feature_forward(att_feats, rsi_feats, att_masks, rsi_masks)  # add prefix feat (att_feats_, rsi_feats_)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, rsi_feats, att_masks, rsi_masks)
        ...
        if opt.get('prefix_feat') == 'rn':
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, prefix_rn)  # add prefix feat
            prefix_feat = utils.repeat_tensors(beam_size, prefix_rn)
        elif opt.get('prefix_feat') == 'rsclip':
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, prefix_clip)  # add prefix feat
            prefix_feat = utils.repeat_tensors(beam_size, prefix_clip)
        elif opt.get('prefix_feat') == 'rsclip+rn':
            prefix_feat_cat = torch.cat((prefix_clip, prefix_rn), dim=1)
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, prefix_feat_cat)  # add prefix feat
            prefix_feat = utils.repeat_tensors(beam_size, prefix_feat_cat)
        else:
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)  # add prefix feat
            prefix_feat = None
            # logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, rsi_feats_)  # add prefix feat
            # rsi_feats_ = utils.repeat_tensors(beam_size, rsi_feats_)
            # prefix_feat = rsi_feats_

        # logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)  # yuan
        ...
        # set_trace()
        self.done_beams = self.beam_search(prefix_feat, state, logprobs, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, opt=opt)   # add prefix feat
    ...
    def _sample(self, fc_feats, att_feats, rsi_feats, att_masks=None, rsi_masks=None, opt={}):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, rsi_feats, att_masks, rsi_masks, opt)
        if group_size > 1:
        ...
        # set_trace()
        if opt.get('use_prefix_feat'):   # add prefix feat
            prefix_clip = self.adapool(att_feats)
            prefix_rn = self.adapool(rsi_feats)
        else:
            prefix_clip = prefix_rn = None
        ...
        seq = fc_feats.new_full((batch_size * sample_n, self.seq_length), self.pad_idx, dtype=torch.long)
        seqLogprobs = fc_feats.new_zeros(batch_size * sample_n, self.seq_length, self.vocab_size + 1)
        # seqLogprobs = fc_feats.new_zeros(batch_size * sample_n, self.seq_length, self.vocab_size)     # GPT2tokenizer
        for t in range(self.seq_length + 1):
            if t == 0:  # input <bos>
                it = fc_feats.new_full([batch_size * sample_n], self.bos_idx, dtype=torch.long)

            if opt.get('prefix_feat') == 'rn':
                logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, prefix_rn,
                                                          output_logsoftmax=output_logsoftmax)     # add prefix feat
            elif opt.get('prefix_feat') == 'rsclip':
                logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, prefix_clip,
                                                          output_logsoftmax=output_logsoftmax)  # add prefix feat
            elif opt.get('prefix_feat') == 'rsclip+rn':
                prefix_feat = torch.cat((prefix_clip, prefix_rn), dim=1)
                logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, prefix_feat,
                                                          output_logsoftmax=output_logsoftmax)  # add prefix feat
            else:
