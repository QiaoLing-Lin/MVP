...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.unk_idx = getattr(opt, 'unk_idx', opt.vocab_size)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        # no prefix feat
        prefix_feat = None
        self.done_beams = self.beam_search(prefix_feat, state, logprobs, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, opt=opt)
        for k in range(batch_size):
            if sample_n == beam_size:
                for _n in range(sample_n):
