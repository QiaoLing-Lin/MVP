...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, mask))   # Q, K, V  x[bs, 49, 512]
...
#
#     def forward(self, x):
#         return  self.Glove_embedding(x)
#
# class Glove_embedding(nn.Module):
#     def __init__(self, glove_dim, d_model, opt):
#         super(Glove_embedding, self).__init__()
#         self.glove_dim = glove_dim
#         self.dict = torch.from_numpy(np.load(opt.glove_embedding_dict)).cuda().to(torch.float32)
#         self.pro = nn.Linear(glove_dim, d_model)
#
#     def forward(self, x):
#         batch_size = x.size(0)
#         x = x.to(torch.long).flatten()
#         x = self.pro(torch.index_select(self.dict, 0, x).view(batch_size, -1, self.glove_dim))
#         return x
...
class TransformerModel(AttModel):
    ...
    def __init__(self, opt):
        ...
        self.h = opt.num_att_heads  #  getattr(opt, 'num_att_heads', 8)
        self.dropout = opt.dropout  #  getattr(opt, 'dropout', 0.1)
